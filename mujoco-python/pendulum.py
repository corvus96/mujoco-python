...
def controller(model, data):
    ...
    if actuator_type == "torque":
        ...
        data.ctrl[0] = -kp * \
            (data.sensordata[0] - in_signal) - \
            kv * (data.sensordata[1] - in_signal)
...
in_signal = 0.0
# Create vectors to plot variables
timevals = []
angular_vel= []
angular_pos = []
input_signal = []
while not glfw.window_should_close(window):
    ...
    while (data.time - simstart < 1.0/60.0):
        ...
        angular_vel.append(data.qpos[0].copy())
        angular_pos.append(data.qvel[0].copy())
        input_signal.append(in_signal)
...
ax[0].plot(timevals, angular_vel, color ="red")
ax[0].plot(timevals, input_signal, color="blue")
ax[0].set_title('angular velocity')
ax[0].set_ylabel('radians / second')

ax[1].plot(timevals, angular_pos, color="red")
ax[1].plot(timevals, input_signal, color="blue")
ax[1].set_xlabel('time (seconds)')
ax[1].set_ylabel('rad')
_ = ax[1].set_title('position')
plt.show()
